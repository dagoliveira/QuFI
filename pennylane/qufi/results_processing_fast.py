...
def compute_QVF_michelson_contrast_single_injection(df, circuit_name, phi, theta):
    ...
    QVF = {}
    QVF['QVF_circuit'] = dfFilter['QVF'].round(3).mean()
    ...
    for i, q in indexes_wires:
        QVF['QVF_index_'+str(i)+'_qubit_'+str(q)] = dfFilter[(dfFilter.indexes==i) & (dfFilter.first_qubit_injected==q)]['QVF'].round(3).mean()
        # if pd.isna(QVF['QVF_qubit_'+str(q)+'_index_'+str(i)]):
        #     print(QVF['QVF_qubit_'+str(q)+'_index_'+str(i)])

    for q in wires:
        QVF['QVF_qubit_'+str(q)] = dfFilter[dfFilter.first_qubit_injected==q]['QVF'].round(3).mean()
...
def read_results_directory_fast(dir, noise=False):
        ...
        for circuit in circuits:
            ...
            for qvf_idx in QVF_list:
                ...
                for theta in theta_list:
                    for phi in phi_list:
                        # print(df[(df.circuit_name==circuit) & (df.first_phi==phi) & (df.first_theta==theta)][qvf_idx].values[0])
                        qvf[circuit][qvf_idx].append(round(df[(df.circuit_name==circuit) & (df.first_phi==phi) & (df.first_theta==theta)][qvf_idx].mean(),3))
...
def compute_index_heatmaps(circs, qvf, savepath="./plots/heatmaps/"):
    """Compute single qubit histograms for single and double FI"""
...
def generate_all_statistics_fast(results, savepath="./plots"):
    ...
    qvf = compute_qubit_heatmaps(circs, f"{savepath}/heatmaps/")
    compute_index_heatmaps(circs, qvf, f"{savepath}/heatmaps_index/")
